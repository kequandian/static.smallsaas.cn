...
export default function index(props) {
    ...
    const { phone, validateCode, password, appid = 'Unicom5G' } = props

    const [showPhoneLogin, setShowPhoneLogin] = useState(true)

    let api = `/api/u/accountProduct/${appid}`
                    ...
                    <CssCart width='200px' margin='auto'>
                        <Container>
                            <Flexbox justify='start' direction='row' align='start-with-last-end' >
                                <Box borderBottom='2px #ad2429 solid' padding='0 4px'>
                                <Text fontSize='16px' color='#ad2419' onClick={() => swtichClick(true)}>联通5G卡</Text>
                                </Box>

                                <Text fontSize='16px' color='#333333' onClick={() => swtichClick(false)}>跨境短信</Text>
                            </Flexbox>
                        </Container>
                    </CssCart>
