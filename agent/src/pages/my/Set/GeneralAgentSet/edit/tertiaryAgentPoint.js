...
export default function index(props) {
    ...
    const [tertiaryAgentPoint, setKeyValues] = useState(queryData.query.tertiaryAgentPoint)

    const title = (queryData.query.text) ? ('设置三级代理团队预存款分成') : ('')
    ...
    function changName() {
        if (tertiaryAgentPoint) {
            updateName({ tertiaryAgentPoint })
        }
    }
            ...
            <Center w='100%' bg='' margin=' 0' padding='0 20px'>
                <from style={{ width: '90%' }} >
                    <CssCart background='#ffffff' margin='20px 0' boxShadow='0 1px 8px rgba(0, 0, 0, 0.08) ' padding='10px 10px 14px 10px'>
                        <input type='number' value={tertiaryAgentPoint || ''}
