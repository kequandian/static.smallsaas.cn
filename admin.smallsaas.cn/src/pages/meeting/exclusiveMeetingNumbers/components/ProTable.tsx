import { getExclusiveMeetingNumbers } from '@/api/meeting';
...
const ProTableV1: React.FC = () => {
  ...
  const { createModalOpen, setCreateModalOpen, onDelExclusive } = useMeeting();
  ...
  useEffect(() => {
    const updataListPub = Pubsub.subscribe('UPDATE-EXCLUSIVEMEETINGNUMBERS', () => {
      // 刷新
      if (proTableV1Ref) {
        // 手动调用刷新
    ...
    },
    {
      ...
      render: (_, record: any) => (
        <Space size={20}>
          ...
          <Confirmation onConfirm={() => onDelExclusive(record.id)} key="del">
  ...
  const request = async (params: any) => {
    ...
    const { data } = await getExclusiveMeetingNumbers({
