import { changeActivityStatus, getActivityList } from '@/api/activityManager';
...
const ProTableV1: React.FC = () => {
  ...
  const { createModalOpen, setCreateModalOpen, onVolunteerActivityDel } = useLauncher();
  const [formData, setFormData] = useState<any>();
  const [modelType, setModelType] = useState<EModelType>(EModelType.ADD);

  useEffect(() => {
    const updataListPub = Pubsub.subscribe('UPDATE-ACTIVITY-TABLE', () => {
      // 刷新
      if (proTableV1Ref) {
        // 手动调用刷新
  ...
  const onView = (data: any) => {
    history.push(`/launcher/activity/activityManager/details/${data?.id}`, data);
  };
  ...
  const onStart = async (data: any) => {
    const res = await changeActivityStatus({
      activityId: data.id,
      status: 2,
    });
    if (res.code === 0) {
  ...
  const columns: ProColumns<any>[] = [
    ...
    },
    {
      ...
      ellipsis: true,
      hideInSearch: true,
      render: (_, i: any) => <Tag>{typeOptions.find((k: any) => k?.value === i?.type)?.label}</Tag>,
    },
    {
      title: '开始时间',
...

    {
      ...
      render: (_, record: any) => (
        <Space size={10}>
          ...
          {record.status === 3 && (
            <Confirmation onConfirm={() => onVolunteerActivityDel(record.id)} key="del">
