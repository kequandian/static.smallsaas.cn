import { getNoticePublishList } from '@/api/baseWork';
...
const ProTableV1: React.FC = () => {
  ...
  const { createModalOpen, setCreateModalOpen, onNoticeAnnouncementDel } = useLauncher();
    ...
    },
    {
      ...
      render: (_, record: any) => (
        <Space size={20}>
          ...
          <Confirmation onConfirm={() => onNoticeAnnouncementDel(record.id)} key="del">
            <a>删除</a>
          </Confirmation>
        </Space>
      ),
  ...
  const request = async (params: any) => {
    const { data } = await getNoticePublishList({
