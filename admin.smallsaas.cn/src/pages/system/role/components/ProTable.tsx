import { getPermissionList } from '@/api/roles';
import { getRolesList } from '@/api/tenantManagement';
...
const ProTableV1: React.FC = () => {
  ...
  const { createModalOpen, setCreateModalOpen, onDelRole } = useSystem();
  ...
  useEffect(() => {
    const updataListPub = Pubsub.subscribe('UPDATE-ROLELIST', () => {
      // 刷新
      if (proTableV1Ref) {
        // 手动调用刷新
    ...
    },
    {
      ...
      render: (_, record: any) => (
        <Space size={20}>
          ...
          <Confirmation onConfirm={() => onDelRole(record.id)} key="del">
