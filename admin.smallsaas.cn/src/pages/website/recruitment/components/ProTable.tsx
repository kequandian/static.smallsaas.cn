...
import { getRecruitmentList, deleteRecruitment } from '@/api/website';
...
const ProTableV1: React.FC = () => {
  ...
  const onDel = async (id: any) => {
    const res = await deleteRecruitment({ id: id });
  ...
  useEffect(() => {
    const updataListPub = Pubsub.subscribe('UPDATE-RECRUITMENT', () => {
      // 刷新
      if (proTableV1Ref) {
        // 手动调用刷新
        proTableV1Ref.current?.reload();
