...
import {
  firmwareOtaPlanDelete,
  firmwareOtaPlanExec,
  firmwareOtaPlanPage,
} from '@/services/mdm/firmwareOtaPlan';
...
const ProTableV1: React.FC = () => {
  ...
  const onDel = async (id: any) => {
    const { code } = await firmwareOtaPlanDelete({ id: id });
  ...
  const onExecute = async (id: any) => {
    const data = await firmwareOtaPlanExec({ id: id });
    if (data?.code === 200) {
  ...
  useEffect(() => {
    const updataListPub = Pubsub.subscribe('UPDATE-FIRMWAREOTAPLAN', () => {
      // 刷新
      if (proTableV1Ref) {
        // 手动调用刷新
        proTableV1Ref.current?.reload();
  ...
  const request = async (params: any) => {
    const { data } = await firmwareOtaPlanPage({
