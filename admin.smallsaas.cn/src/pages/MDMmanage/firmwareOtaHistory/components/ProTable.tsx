...
import {
  firmwareOtaHistoryDelete,
  firmwareOtaHistoryPage,
} from '@/services/mdm/firmwareOtaHistory';
...
const ProTableV1: React.FC = () => {
  ...
  const onDel = async (id: any) => {
    const { code } = await firmwareOtaHistoryDelete({ id: id });
  ...
  useEffect(() => {
    const updataListPub = Pubsub.subscribe('UPDATE-OTAPLAN', () => {
      // 刷新
      if (proTableV1Ref) {
        // 手动调用刷新
        proTableV1Ref.current?.reload();
  ...
  const request = async (params: any) => {
    const { data } = await firmwareOtaHistoryPage({
      ...
      name: params?.name,
      moduleIdentify: params?.moduleIdentify,
