...
import { channelDelete, channelPage } from '@/services/mdm/channel';
...
const ProTableV1: React.FC = () => {
  ...
  const columns: ProColumns<any>[] = [
...

    {
      ...
      ellipsis: true,
      render: (_, record: any) => {
        return record.strategyNames?.map((item: any) => item).join('、');
      },
    },
    {
      title: '应用组织',
      ...
      ellipsis: true,
      render: (_, record: any) => {
        return record.orgNames?.map((item: any) => item).join('、');
      },
    },
  ...
  useEffect(() => {
    const updataListPub = Pubsub.subscribe('UPDATE-CHANNEL', () => {
      // 刷新
      if (proTableV1Ref) {
        // 手动调用刷新
        proTableV1Ref.current?.reload();
