...
// 单个ipv4
export const IPV4Single =
  /^((25[0-5]|2[0-4]\d|[01]?\d\d?)\.){3}(25[0-5]|2[0-4]\d|[01]?\d\d?)(\/(\d|[1-2]\d|3[0-2]))?$/;

// 端口
export const PORTSingle =
  /^([0-9]|[1-9]\d{1,3}|[1-5]\d{4}|6[0-4]\d{4}|65[0-4]\d{2}|655[0-2]\d|6553[0-5])$/;
// 单个ipv4/mask 校验
export const IPV4SingleMask = (ip: string) => {
  console.log(ip);
 ...
 * @param {*} s
 */
export function IdCard(UUserCard: any) {
  let briday =
    UUserCard.substring(6, 10) +
    '-' +
    UUserCard.substring(10, 12) +
    '-' +
    UUserCard.substring(12, 14); //出生日期
  console.log(briday);
  let sex = '' as any; //性别
...
export function formatTen(num: any) {
  return num > 9 ? num + '' : '0' + num;
}

//标准时间转年月日---
export function TimeYY(date: any) {
...
//倒计时
export function daojishi(endTime: any, startTime: any) {
...
//时间戳转时间
// 时间戳转换成时间
export function timestampToTime(cjsj: any) {
...
export function timestampToTimes(cjsj: any) {
  let date = new Date(cjsj); //时间戳为10位需*1000，时间戳为13位的话不需乘1000
  let Y = date.getFullYear() + '-';
  let M = (date.getMonth() + 1 < 10 ? '0' + (date.getMonth() + 1) : date.getMonth() + 1) + '-';
  let D = (date.getDate() < 10 ? '0' + date.getDate() : date.getDate()) + ' ';
